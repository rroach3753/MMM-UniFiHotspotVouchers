...
2. **Add debug logging support** (aids troubleshooting)
3. **Fix potential race condition in `initialize()`** (edge case but good practice)

### Low Priority

6. Fix `createdAt` null handling in sort
2. Remove unused CSS class (`.unifi-hotspot__row--expired`)
3. Improve string concatenation in HTTP response buffering
4. Add initialization state tracking to prevent overlapping calls
